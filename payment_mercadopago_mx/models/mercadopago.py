...
class AcquirerMercadopago(models.Model):
    ...
    #    """ Override of payment to unlist MercadoPago Méxicoacquirers when the currency is not supported. """
    ...
    #mercadopago_client_id = fields.Char('MercadoPago MéxicoClient Id',required_if_provider='mercadopago')
    mercadopago_client_id = fields.Char('MercadoPago MéxicoClient Id', size=256)
    #mercadopago_secret_key = fields.Char('MercadoPago MéxicoSecret Key',required_if_provider='mercadopago')
    mercadopago_secret_key = fields.Char('MercadoPago MéxicoSecret Key', size=256)

    #mercadopago_email_account = fields.Char('MercadoPago MéxicoEmail ID', required_if_provider='mercadopago')
    mercadopago_email_account = fields.Char('MercadoPago MéxicoEmail ID', size=256)

    mercadopago_seller_account = fields.Char(
            'MercadoPago MéxicoMerchant ID',
            size=256,
            help='The Merchant ID is used to ensure communications coming from MercadoPago Méxicoare valid and secured.')

    mercadopago_use_ipn = fields.Boolean('Use IPN', help='MercadoPago MéxicoInstant Payment Notification')
        ...
        #for (company_id, company_mercadopago_account) in res:
            if company_mercadopago_account:
                ...
                if company_mercadopago_ids:
                    ...
                    mercadopago_view = self.env['ir.model.data'].get_object(cr, uid, 'payment_mercadopago_mx', 'mercadopago_acquirer_button')
                    self.create({
                        'name': 'MercadoPago',
    ...
    def mercadopago_mx_form_generate_values(self, values, tx):
...
class TxMercadoPago(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        return self.acquirer_id.mercadopago_mx_form_generate_values(processing_values,tx=self)
...
#charged_back (estado terminal) 	Se realizó un contracargo en la tarjeta de crédito.
    ...
    def _process_feedback_data(self, data):
        ...
        if status in ['approved', 'processed']:
            _logger.info('Validated MercadoPago Méxicopayment for tx %s: set as done' % (self.reference))
            ...
            _logger.info('Received notification for MercadoPago Méxicopayment %s: set as pending' % (self.reference))
            ...
            _logger.info('Received notification for MercadoPago Méxicopayment %s: set as cancelled' % (self.reference))
            if (self.state not in ['cancel']):
                data.update(state='cancel', state_message=data.get('cancel_reason', ''))
            self._set_canceled()
            #self.sudo().write(data)
        else:
            error = 'Received unrecognized status for MercadoPago Méxicopayment %s: %s, set as error' % (self.reference, status)
            _logger.info(error)
    ...
    def _mercadopago_mx_s2s_send(self, values, cc_values, context=None):
        ...
        headers = {
            ...
            'Authorization': 'Bearer %s' % tx.acquirer_id._mercadopago_mx_s2s_get_access_token()[tx.acquirer_id.id],
        }
        data = {
            'intent': 'sale',
            'transactions': [{
    ...
    def _mercadopago_mx_s2s_validate(self, tx, data, context=None):
        ...
        if status in ['approved']:
            ...
            _logger.info('Received notification for MercadoPago Méxicos2s payment %s: set as pending' % (tx.reference))
            tx.write({
                'state': 'pending',
            ...
            error = 'Received unrecognized status for MercadoPago Méxicos2s payment %s: %s, set as error' % (tx.reference, status)
            _logger.info(error)
            tx.write({
                'state': 'error',
                # 'state_message': error,
